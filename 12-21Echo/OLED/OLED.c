...
{
    HAL_GPIO_WritePin(GPIOB,GPIO_PIN_9, Y);
}   
...
{
	OLED_W_SDA(GPIO_PIN_SET);		//释放SDA，确保SDA为高电平
	OLED_W_SCL(GPIO_PIN_SET);		//释放SCL，确保SCL为高电平
	OLED_W_SDA(GPIO_PIN_RESET);		//在SCL高电平期间，拉低SDA，产生起始信号
	OLED_W_SCL(GPIO_PIN_RESET);		//起始后把SCL也拉低，即为了占用总线，也为了方便总线时序的拼接
}
...
{
	OLED_W_SDA(GPIO_PIN_RESET);		//拉低SDA，确保SDA为低电平
	OLED_W_SCL(GPIO_PIN_SET);		//释放SCL，使SCL呈现高电平
	OLED_W_SDA(GPIO_PIN_SET);		//在SCL高电平期间，释放SDA，产生终止信号
}
...
//		OLED_W_SDA(!!(Byte & (0x80 >> i)));
		...
		OLED_W_SCL(GPIO_PIN_SET);	//释放SCL，从机在SCL高电平期间读取SDA
		OLED_W_SCL(GPIO_PIN_RESET);	//拉低SCL，主机开始发送下一位数据
	}
